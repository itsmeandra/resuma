 ...
 */
export function getResumeAnalysisSchema(hasJobDescription: boolean): Schema {
    const schema: Schema = {
        type: Type.OBJECT,
        properties: {
            detectedRole: {
    ...
    if (hasJobDescription) {
        schema.properties!.keywordAnalysis = {
            type: Type.OBJECT,
            nullable: true,
            description: 'Hasil analisis kecocokan keyword (TIDAK BOLEH NULL JIKA TARGET JOB DESCRIPTION DIBERIKAN)',
            properties: {
                matchScore: { type: Type.INTEGER, description: 'Persentase kecocokan keyword (0-100)' },
                matchedKeywords: { type: Type.ARRAY, items: { type: Type.STRING }, description: 'Daftar keyword penting dari JD yang DITEMUKAN di resume' },
                missingKeywords: { type: Type.ARRAY, items: { type: Type.STRING }, description: 'Daftar keyword penting dari JD yang HILANG / TIDAK ADA di resume' },
            },
            required: ['matchScore', 'matchedKeywords', 'missingKeywords'],
        };
        schema.required!.push('keywordAnalysis');
    }
    return schema;
};
...
export function buildResumeReviewPrompt(
    ...
    const jdContext = jobDescription && jobDescription.trim() !== ''
        ? `\n[TARGET JOB DESCRIPTION]:\n${jobDescription}\n\nINSTRUKSI KHUSUS: Target Job Description telah diberikan. Anda WAJIB mengisi properti 'keywordAnalysis'. Ekstrak kata kunci utama (hard skills, soft skills, tools) dari JD, bandingkan dengan resume, hitung skor kecocokan (0-100), dan daftarkan mana yang 'matched' dan 'missing'.`
        : '\n[TARGET JOB DESCRIPTION]: Tidak disediakan. Anda TIDAK PERLU mengisi properti "keywordAnalysis" (kembalikan null atau abaikan bagian tersebut). Evaluasi berdasarkan standar industri universal.';
